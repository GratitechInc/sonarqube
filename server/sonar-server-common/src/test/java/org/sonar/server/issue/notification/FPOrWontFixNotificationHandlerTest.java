...
public class FPOrWontFixNotificationHandlerTest {
  ...
  public void deliver_has_no_effect_if_emailNotificationChannel_is_disabled() {
    ...
    Set<IssuesChangesNotification> notifications = IntStream.range(0, 1 + new SecureRandom().nextInt(10))
  ...
  public void deliver_parses_every_notification_in_order() {
    Set<IssuesChangesNotification> notifications = IntStream.range(0, 5 + new SecureRandom().nextInt(10))
      .mapToObj(i -> mock(IssuesChangesNotification.class))
      .collect(toSet());
  ...
  public void deliver_fails_with_IAE_if_serializer_throws_IAE() {
    Set<IssuesChangesNotification> notifications = IntStream.range(0, 3 + new SecureRandom().nextInt(10))
      .mapToObj(i -> mock(IssuesChangesNotification.class))
      .collect(toSet());
    when(emailNotificationChannel.isActivated()).thenReturn(true);
  ...
  public void deliver_has_no_effect_if_no_issue_has_new_resolution() {
    ...
    Set<IssuesChangesNotification> notifications = IntStream.range(0, 2 + new SecureRandom().nextInt(5))
  ...
  public void deliver_checks_by_projectKey_if_notifications_have_subscribed_assignee_to_FPorWontFix_notifications(String newResolution) {
    ...
    Stream<IssuesChangesNotificationBuilder> project1Notifications = IntStream.range(0, 1 + new SecureRandom().nextInt(2))
    ...
    Stream<IssuesChangesNotificationBuilder> project2Notifications = IntStream.range(0, 1 + new SecureRandom().nextInt(2))
      .mapToObj(j -> new IssuesChangesNotificationBuilder(
    ...
    Stream<IssuesChangesNotificationBuilder> project3And4Notifications = IntStream.range(0, 1 + new SecureRandom().nextInt(2))
      .mapToObj(j -> new IssuesChangesNotificationBuilder(
        Stream.concat(
  ...
  public void deliver_does_not_send_email_request_for_notifications_a_subscriber_is_the_changeAuthor_of(String newResolution) {
    ...
    Set<IssuesChangesNotificationBuilder> subscriber1Notifications = IntStream.range(0, 1 + new SecureRandom().nextInt(2))
      .mapToObj(j -> new IssuesChangesNotificationBuilder(
    ...
    Set<IssuesChangesNotificationBuilder> subscriber1and2Notifications = IntStream.range(0, 1 + new SecureRandom().nextInt(2))
      .mapToObj(j -> new IssuesChangesNotificationBuilder(
        Stream.concat(
    ...
    Set<IssuesChangesNotificationBuilder> subscriber2Notifications = IntStream.range(0, 1 + new SecureRandom().nextInt(2))
      .mapToObj(j -> new IssuesChangesNotificationBuilder(
        randomIssues(t -> t.setProject(project).setNewResolution(newResolution).setAssignee(subscriber2)).collect(toSet()),
    ...
    Set<IssuesChangesNotificationBuilder> subscriber2And3Notifications = IntStream.range(0, 1 + new SecureRandom().nextInt(2))
      .mapToObj(j -> new IssuesChangesNotificationBuilder(
        Stream.concat(
          randomIssues(t -> t.setProject(project).setNewResolution(newResolution).setAssignee(subscriber2)),
    ...
    Set<IssuesChangesNotificationBuilder> otherChangeAuthorNotifications = IntStream.range(0, 1 + new SecureRandom().nextInt(2))
    ...
    int deliveredCount = new SecureRandom().nextInt(200);
